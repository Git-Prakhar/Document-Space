...
const RECENT_CHATS = [
  ...
  { id: "c2", title: "Quarterly report analysis",  time: "Yesterday", sources: 1 },
...
export default function ChatPage() {
  ...
  const [sources, setSources] = useState([
    ...
    { id: "s3", type: "website", name: "ipcc.ch/report/ar6",      status: "loading" },
    { id: "s4", type: "audio",   name: "interview_recording.mp3", status: "error"   },
  ]);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      setSources((prev) =>
        prev.map((s) => s.status === "loading" ? { ...s, status: "ready" } : s)
      );
    }, 3500);
  ...
  const addSource = ({ type, name }) => {
    ...
    setTimeout(() => {
      setSources((prev) =>
        prev.map((s) => s.id === newSrc.id ? { ...s, status: Math.random() > 0.2 ? "ready" : "error" } : s)
      );
  ...
  const sendMessage = () => {
    ...
    // Simulate streaming
    const reply = "I've analyzed the sources you've provided. Based on the documents and recordings available, the mitigation strategies discussed include carbon capture technologies, renewable energy transitions, and policy-level interventions at the international level. The audio recording specifically mentions a 40% reduction target by 2035 as the most feasible pathway according to current modelling.";
    let i = 0;
    const iv = setInterval(() => {
      i += Math.floor(Math.random() * 6) + 2;
      const chunk = reply.slice(0, i);
      setMessages((prev) => prev.map((m) => m.id === asstMsg.id ? { ...m, text: chunk, streaming: i < reply.length } : m));
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden"
      style={{ background: "#0e0e12", fontFamily: "'DM Sans', sans-serif", color: "#e4e4f0" }}>

      {/* Google font import */}
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Mono:wght@400;500&display=swap');`}</style>

      {/* ── Sidebar ── */}
      <aside className="flex-shrink-0 flex flex-col border-r transition-all duration-300 overflow-hidden"
        style={{ width: sidebarOpen ? 240 : 0, borderColor: "rgba(255,255,255,0.06)", background: "#111115" }}>
        <div className="flex flex-col h-full" style={{ width: 240 }}>
          ...
          <div className="flex items-center gap-2.5 px-5 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
            ...
            <span className="text-sm font-semibold tracking-tight" style={{ color: "#e4e4f0" }}>SourceChat</span>
          </div>

          {/* New Chat button */}
          <div className="px-3 pt-3 pb-2">
            <button className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all hover:bg-white/5"
              style={{ color: "#9090a8", border: "1px dashed rgba(255,255,255,0.1)" }}>
              <Icon.NewChat /><span>New chat</span>
            </button>
          </div>

          {/* Recent chats */}
          <div className="flex-1 overflow-y-auto px-3 py-1 space-y-0.5">
            <p className="text-[10px] uppercase tracking-widest px-2 py-1.5" style={{ color: "#555566" }}>Recent</p>
            {RECENT_CHATS.map((chat) => (
              <button key={chat.id}
                onClick={() => { setActiveChat(chat.id); setChatTitle(chat.title); setTitleDraft(chat.title); }}
                className={`w-full text-left px-2.5 py-2.5 rounded-lg transition-all group ${activeChat === chat.id ? "bg-white/7" : "hover:bg-white/4"}`}>
                <p className={`text-xs font-medium truncate mb-0.5 ${activeChat === chat.id ? "text-neutral-100" : "text-neutral-400 group-hover:text-neutral-200"}`}>
                  {chat.title}
                </p>
                <div className="flex items-center gap-2">
                  <span className="text-[10px]" style={{ color: "#555566" }}>{chat.time}</span>
                  {chat.sources > 0 && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded"
                      style={{ background: "rgba(245,158,11,0.1)", color: "#9a7030" }}>
                      {chat.sources} src
                    </span>
                  )}
                </div>
          ...
          <div className="px-3 py-3 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
            <div className="flex items-center gap-2.5 px-2">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
                style={{ background: "linear-gradient(135deg,#f59e0b44,#ef444433)", color: "#f59e0b" }}>J</div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-neutral-300 truncate">Jamie Chen</p>
                <p className="text-[10px]" style={{ color: "#555" }}>Free plan</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="flex items-center gap-3 px-4 h-14 flex-shrink-0 border-b"
          ...
          <div className="flex-1 flex items-center gap-2 min-w-0">
            {editingTitle ? (
              ...
              <button onClick={() => { setEditingTitle(true); setTitleDraft(chatTitle); setTimeout(() => titleInputRef.current?.select(), 50); }}
                className="group flex items-center gap-1.5 min-w-0">
                <span className="text-sm font-semibold text-neutral-100 truncate max-w-xs">{chatTitle}</span>
          ...
          <div className="relative flex-shrink-0">
            <button onClick={() => setSourcesOpen(!sourcesOpen)}
              ...
              style={{
                background: sourcesOpen ? "rgba(245,158,11,0.12)" : "rgba(255,255,255,0.05)",
              ...
              {sources.some((s) => s.status === "error") && (
                <span className="text-red-400"><Icon.AlertTriangle /></span>
              )}
        ...
        {/* ── Chat area ── */}
        <main className="flex-1 overflow-y-auto px-4 py-6 space-y-5" style={{ background: "#0e0e12" }}>
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-neutral-300 mb-1">Start by adding sources</p>
                <p className="text-xs text-neutral-600">Upload files, images, audio, or add a website link to chat with your content.</p>
              </div>
            </div>
          ) : (
        ...
        {/* ── Input bar ── */}
        <footer className="flex-shrink-0 px-4 py-3 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", background: "#111115" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={sendMessage} disabled={!input.trim()}
              ...
              style={{
                ...
                boxShadow: input.trim() ? "0 4px 16px rgba(245,158,11,0.3)" : "none",
              }}>
              <Icon.Send />
            </button>
          </div>
